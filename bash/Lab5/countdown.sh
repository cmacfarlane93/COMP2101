...
#function for handling interrupt signal
function noInterrupts {
	      #reset numberOfSleeps back to 10
	      numberOfSleeps=10
	      #telling user they cannot use interrupt signal
	      echo "You are not allowed to interrupt"
}
#function for handling quit signal
function secretExit {
        #notify user they have found secret way to end script
	      echo "You have found the secret way to end this script, exiting immediately."
	      #exiting immediately
        exit
}
...
#listening for interrupt signal to run noInterrupts function
trap noInterrupts SIGINT
#listening for quit signal to run secretExit function
trap secretExit SIGQUIT
